...
class GameLog private constructor(
  ...
  private var _context: Map<String, Any> = mapOf(),
) {
  ...
  fun logEvent(eventType: String, block: ContextBuilder.() -> Unit = {}) {
    val context = ContextBuilder(_context).also(block).build()
    with(logger.atInfo()) {
  ...
  suspend fun logScope(block: suspend GameLog.() -> Unit) {
    GameLog(_events, _context).block()
  }
}