...
object GameEvent {
  val GameEvent.description: String
    get() = when (this) {
      ...
      is GameEvent.BlockAttempted -> "$blocker attempted to block ${action.player}'s ${attemptOf(action.type)} on behalf of ${
        the(
          influence
        )
      }."
      ...
      is GameEvent.ActionBlocked -> "$blocker blocked ${action.player}'s ${attemptOf(action.type)} on behalf of ${
        the(
          influence
        )
      }."
  ...
  private fun performed(actionType: Action.Type, target: Player?) = when (actionType) {
  ...
  private fun attempted(actionType: Action.Type, target: Player?) = when (actionType) {
  ...
  private fun attemptOf(actionType: Action.Type) = when (actionType) {
